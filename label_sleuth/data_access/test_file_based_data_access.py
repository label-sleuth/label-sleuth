...
class TestFileBasedDataAccess(unittest.TestCase):
    ...
    def test_unset_labels(self):
        ...
        labels_count = self.data_access.get_label_counts(workspace_id, dataset_name, category)
        self.assertGreater(labels_count[True], 0)
        self.data_access.unset_labels(workspace_id, category, [x[0] for x in texts_and_labels_list])
        labels_count_after_unset = self.data_access.get_label_counts(workspace_id, dataset_name, category)
    ...
    def test_get_unlabeled_text_elements(self):
        ...
        sampled_texts_res = self.data_access.get_unlabeled_text_elements(workspace_id, dataset_name, category,
                                                                         sample_all)
    ...
    def test_get_labeled_text_elements(self):
        ...
        sampled_texts_res = self.data_access.get_labeled_text_elements(workspace_id, dataset_name, category,
                                                                       sample_all)
    ...
    def test_sample_by_query_text_elements(self):
        ...
        # add labels info for a single doc
        uri_to_label_dict = {doc.text_elements[0].uri: {category: Label(label=LABEL_POSITIVE)},
                             # 1st sent does not match query
                             doc.text_elements[1].uri: {
                                 category: Label(label=LABEL_POSITIVE)}}  # 2nd sent does match query
        ...
        sampled_texts_res = self.data_access.get_labeled_text_elements(workspace_id, dataset_name, category, sample_all,
                                                                       query)
    ...
    def test_multithread_async_set_labels(self):
        ...
        self.assertDictEqual(dict(all_from_threads), dict(
            self.data_access.get_label_counts(workspace_id, dataset_name, category)))
        self.data_access.delete_dataset(dataset_name)
    ...
    def set_labels_thread(self, categories, category, corpus, workspace_id, start, stop, idx, results):
        ...
        true_count = len(
            [1 for _, label_dict in sentences_and_labels_all.items() if label_dict[category].label == LABEL_POSITIVE])
        false_count = len(
            [1 for _, label_dict in sentences_and_labels_all.items() if label_dict[category].label == LABEL_NEGATIVE])
    ...
    def test_get_label_counts(self):
        ...
        # add labels info for a single doc
        selected_doc = docs[0]
        uri_to_label_dict = add_random_labels_to_document(selected_doc, 5, [category_to_count, "another category 1",
                                                                            "another category 2"])
    ...
    def test_duplicates_removal(self):
        ...
        # unset labels
        self.data_access.unset_labels(workspace_id, category, [elem.uri for elem in all_without_dups])
        labels_count = self.data_access.get_label_counts(workspace_id, dataset_name, category)
        ...
        self.data_access.unset_labels(workspace_id, category, [elem.uri for elem in all_elements])
        ...
        labels_count = self.data_access.get_label_counts(workspace_id, dataset_name, category)
        sampled = \
            self.data_access.get_labeled_text_elements(workspace_id, dataset_name, category, 10 ** 6,
                                                       remove_duplicates=True)['results']
        ...
        labels_count_no_dups = self.data_access.get_label_counts(workspace_id, dataset_name, category,
