...
class OrchestratorApi:
    ...
    def query(self, workspace_id: str, dataset_name: str, category_name: str, query_regex: str, sample_size: int = sys.maxsize,
    ...
    def set_labels(self, workspace_id: str, uri_to_label: Mapping[str, Mapping[str, Label]], apply_to_duplicate_texts=True,
    ...
    def unset_labels(self, workspace_id: str, category_name, uris: Sequence[str], apply_to_duplicate_texts=True):
        ...
        the URIs provided.
        """
        self.data_access.unset_labels(workspace_id, category_name, uris, apply_to_duplicate_texts=apply_to_duplicate_texts)
    ...
    def get_all_unlabeled_text_elements(self, workspace_id, dataset_name, category) -> List[TextElement]:
        ...
        :return: a list of TextElement objects
        """
        return self.data_access.get_unlabeled_text_elements(workspace_id, dataset_name, category, sample_size=sys.maxsize,
                                                       remove_duplicates=False)['results']
    ...
    def get_label_counts(self, workspace_id: str, dataset_name: str, category_name: str, remove_duplicates=False):
        ...
        :return:
        """
        return self.data_access.get_label_counts(workspace_id, dataset_name, category_name, remove_duplicates=remove_duplicates)
    ...
    def _calculate_iteration_statistics(self, workspace_id, category_name, iteration_index, predictions: Sequence[Prediction]):
        """
    ...
    def _calculate_active_learning_recommendations(self, workspace_id, dataset_name, category_name, count, iteration_index: int):
        """
        ...
        """
        active_learner = self.active_learning_factory.get_active_learner(CONFIGURATION.active_learning_strategy)
    ...
    def train_if_recommended(self, workspace_id: str, category_name: str, force=False) -> Union[None, str]:
        ...
        try:
            ...
            changes_since_last_model = self.orchestrator_state.get_label_change_count_since_last_train(workspace_id,
                                                                                                      category_name)
            if force or (LABEL_POSITIVE in label_counts
                ...
                model_id = self.run_iteration(workspace_id=workspace_id, category_name=category_name, model_type=model_type,
    ...
    def infer(self, workspace_id: str, category_name: str, elements_to_infer: Sequence[TextElement], iteration_index: int = None,
    ...
    def sample_elements_by_prediction(self, workspace_id, category, sample_size: int = sys.maxsize, unlabeled_only=False,
    ...
    def get_progress(self, workspace_id: str, dataset_name: str, category: str):
        ...
        if category_label_counts[LABEL_POSITIVE]:
            changed_since_last_model_count = self.orchestrator_state.get_label_change_count_since_last_train(workspace_id, category)
            ...
            if workspace.dataset_name == dataset_name:
                ...
                for category_name, category in workspace.categories.items():
                    if CONFIGURATION.apply_labels_to_duplicate_texts:
                        ...
                        labeled_elements = self.data_access.get_labeled_text_elements(workspace_id, dataset_name, category_name)['results']
