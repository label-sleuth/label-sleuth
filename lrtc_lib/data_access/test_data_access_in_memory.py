...
class TestDataAccessInMemory(unittest.TestCase):
    ...
    def test_add_documents_and_get_documents(self):
        ...
        doc = generate_corpus(TestDataAccessInMemory.data_access,dataset_name)[0]
        doc_in_memory = TestDataAccessInMemory.data_access.get_documents(None,dataset_name, [doc.uri])[0]
        # compare all fields
        ...
        TestDataAccessInMemory.data_access.delete_dataset(dataset_name)
    ...
    def test_set_labels_and_get_documents(self):
        ...
        TestDataAccessInMemory.data_access.set_labels(workspace_id, uri_to_label)

        doc_with_labels_info = TestDataAccessInMemory.data_access.get_documents(workspace_id, dataset_name, [doc.uri])
    ...
    def test_unset_labels(self):
        ...
        TestDataAccessInMemory.data_access.set_labels(workspace_id, dict(texts_and_labels_list))

        labels_count = TestDataAccessInMemory.data_access.get_label_counts(workspace_id, dataset_name, category)
        self.assertGreater(labels_count[True], 0)
        TestDataAccessInMemory.data_access.unset_labels(workspace_id, category, [x[0] for x in texts_and_labels_list])
        labels_count_after_unset = TestDataAccessInMemory.data_access.get_label_counts(workspace_id, dataset_name, category)
    ...
    def test_get_all_document_uris(self):
        ...
        docs = generate_corpus(TestDataAccessInMemory.data_access,dataset_name, random.randint(1, 10))
        docs_uris_in_memory = TestDataAccessInMemory.data_access.get_all_document_uris(dataset_name)
    ...
    def test_get_all_text_elements_uris(self):
        ...
        docs = generate_corpus(TestDataAccessInMemory.data_access,dataset_name, random.randint(1, 10))
        text_elements_uris_in_memory = TestDataAccessInMemory.data_access.get_all_text_elements_uris(dataset_name)
    ...
    def test_get_all_text_elements(self):
        ...
        docs = generate_corpus(TestDataAccessInMemory.data_access,dataset_name, random.randint(1, 10))
        text_elements_found = TestDataAccessInMemory.data_access.get_all_text_elements(dataset_name)
        ...
        del TestDataAccessInMemory.data_access.ds_in_memory[dataset_name]
    ...
    #     sample_size = 5
    #     generate_corpus(TestDataAccessInMemory.data_access,dataset_name, 10)
    #     sampled_texts_res = TestDataAccessInMemory.data_access.get_text_elements(dataset_name, sample_size)
    #     self.assertEqual(sample_size, len(sampled_texts_res['results']))
    #
    #     sample_all = 10 ** 100  # a huge sample_size to sample all elements
    #     sampled_texts_res = TestDataAccessInMemory.data_access.get_text_elements(dataset_name, sample_all)
    ...
    #     self.assertEqual(len(TestDataAccessInMemory.data_access.get_all_text_elements_uris(dataset_name)), sampled_texts_res['hit_count'],
    ...
    def test_get_text_elements(self):
        def sample_and_check_that_labels_match(doc, orig_dict):
            sampled_texts_res = TestDataAccessInMemory.data_access.get_text_elements(workspace_id, dataset_name, sample_all)
        ...
        sample_all = 10 ** 100  # a huge sample_size to sample all elements
        docs = generate_corpus(TestDataAccessInMemory.data_access,dataset_name, 5)
        ...
        TestDataAccessInMemory.data_access.set_labels(workspace_id, texts_and_labels_dict)
    ...
    def test_get_unlabeled_text_elements(self):
        ...
        category = 'Autobots'
        sample_all = 10 ** 100  # a huge sample_size to sample all elements
        docs = generate_corpus(TestDataAccessInMemory.data_access,dataset_name, 2)
        # add labels info for a single doc
        selected_doc = docs[0]
        texts_and_labels_list = add_random_labels_to_document(selected_doc, 5, [category])
        TestDataAccessInMemory.data_access.set_labels(workspace_id, texts_and_labels_list)

        sampled_texts_res = TestDataAccessInMemory.data_access.get_unlabeled_text_elements(workspace_id, dataset_name, category, sample_all)
    ...
    def test_get_labeled_text_elements(self):
        ...
        sampled_texts_res = TestDataAccessInMemory.data_access.get_labeled_text_elements(workspace_id, dataset_name, category, sample_all)
    ...
    def test_sample_by_query_text_elements(self):
        ...
        query = 'sentence'
        sample_all = 10 ** 100  # a huge sample_size to sample all elements
        doc = generate_corpus(TestDataAccessInMemory.data_access,dataset_name, 1)[0]
        ...
        TestDataAccessInMemory.data_access.set_labels(workspace_id, uri_to_label_dict)
    ...
    def test_query_text_elements_pagination(self):
        ...
        query = 'sentence'
        sample_all = 10 ** 100  # a huge sample_size to sample all elements
        generate_corpus(TestDataAccessInMemory.data_access,dataset_name, 5)
        ...
        TestDataAccessInMemory.data_access.delete_all_labels(workspace_id,dataset_name)
        TestDataAccessInMemory.data_access.delete_dataset(dataset_name)
        # texts_and_labels_dict = dict(texts_and_labels_list)
        # for sampled_text in sampled_texts_res['results']:
        #     self.assertIn(sampled_text.uri, texts_and_labels_dict.keys(),
    ...
    def test_multithread_async_set_labels(self):
        ...
        category = 'Decepticons'
        categories = [category]
        corpus = generate_corpus(TestDataAccessInMemory.data_access,dataset_name, num_of_documents=NUM_DOCS)
        batch_size = 10
        ...
        self.assertDictEqual(dict(all_from_threads),dict(TestDataAccessInMemory.data_access.get_label_counts(workspace_id,dataset_name,category)))
        TestDataAccessInMemory.data_access.delete_dataset(dataset_name)
    ...
    def test_get_label_counts(self):
        ...
        category_label_counts = TestDataAccessInMemory.data_access.get_label_counts(workspace_id, dataset_name, category_to_count)
    ...
    def test_get_text_elements_by_id(self):
        ...
        all_elements = TestDataAccessInMemory.data_access.get_text_elements_by_uris(workspace_id, dataset_name, uris)
    ...
    def test_duplicates_removal(self):
        ...
        generate_corpus(TestDataAccessInMemory.data_access,dataset_name, 1, add_duplicate=True)
        category = 'cat1'
        all_elements = TestDataAccessInMemory.data_access.get_all_text_elements(dataset_name)
        all_elements2 = TestDataAccessInMemory.data_access.get_text_elements(workspace_id, dataset_name, 10**6, remove_duplicates=False)['results']
        self.assertListEqual(all_elements, all_elements2)
        all_without_dups = TestDataAccessInMemory.data_access.get_text_elements(workspace_id, dataset_name, 10**6, remove_duplicates=True)['results']
        ...
        TestDataAccessInMemory.data_access.set_labels(workspace_id, uri_to_label_dict, apply_to_duplicate_texts=False)
        labels_count = TestDataAccessInMemory.data_access.get_label_counts(workspace_id, dataset_name, category)
        self.assertEqual(labels_count[LABEL_POSITIVE], len(all_without_dups))
        # unset labels
        TestDataAccessInMemory.data_access.unset_labels(workspace_id, category, [elem.uri for elem in all_without_dups])
        labels_count = TestDataAccessInMemory.data_access.get_label_counts(workspace_id, dataset_name, category)
        self.assertEqual(labels_count[LABEL_POSITIVE], 0)
        # set labels with propagating to duplicates
        TestDataAccessInMemory.data_access.set_labels(workspace_id, uri_to_label_dict, apply_to_duplicate_texts=True)
        labels_count = TestDataAccessInMemory.data_access.get_label_counts(workspace_id, dataset_name, category)
        self.assertEqual(labels_count[LABEL_POSITIVE], len(all_elements))
        TestDataAccessInMemory.data_access.unset_labels(workspace_id, category, [elem.uri for elem in all_elements])
        ...
        TestDataAccessInMemory.data_access.set_labels(workspace_id, uri_to_label_dict, apply_to_duplicate_texts=False)
        labels_count = TestDataAccessInMemory.data_access.get_label_counts(workspace_id, dataset_name, category)
        sampled = TestDataAccessInMemory.data_access.get_labeled_text_elements(workspace_id, dataset_name, category, 10**6,
        ...
        labels_count_no_dups = TestDataAccessInMemory.data_access.get_label_counts(workspace_id, dataset_name, category,
