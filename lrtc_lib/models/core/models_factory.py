        ...
        try:
            if model_type == ModelTypes.RAND:
                ...
                if model_type not in self.loaded_models:
                    ...
                    self.loaded_models[model_type] = HFTransformers(self.models_background_jobs_manager,pretrained_model='bert-base-uncased')
                ...
                if model_type not in self.loaded_models:
                    self.loaded_models[model_type] = \
                        Ensemble(model_types=[ModelTypes.SVM_OVER_BOW,
                                              ModelTypes.SVM_OVER_GLOVE],models_background_jobs_manager=self.models_background_jobs_manager)
                ...
                if model_type not in self.loaded_models:
                    self.loaded_models[model_type] = NaiveBayes(RepresentationType.GLOVE,self.models_background_jobs_manager)
            elif model_type == ModelTypes.NB_OVER_BOW:
                from lrtc_lib.models.naive_bayes import NaiveBayes
                if model_type not in self.loaded_models:
                    self.loaded_models[model_type] = NaiveBayes(RepresentationType.BOW,self.models_background_jobs_manager)
            else:
